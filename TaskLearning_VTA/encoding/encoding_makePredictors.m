function [ X, encodingData ] = encoding_makePredictors( trialData, trials, t, params )
...
init.categorical = zeros(size(t));
init.num = NaN(size(t));
init.choice = zeros(size(t));
...
predictors = getTrialDataByTime(trialData, t, params.smoothing_window, params.initFcn_position);
...
termIdx.B0 = 1;  %1st term reserved for intercept
for P = pNames
    ...
    predictorIdx.(P) = size(X,2) + (1:nTerms);
    termIdx.(P) = predictorIdx.(P) + 1; %1st term reserved for intercept
