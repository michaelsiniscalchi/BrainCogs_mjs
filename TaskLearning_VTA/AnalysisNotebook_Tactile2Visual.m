...
%Exclude Alternation sessions (small number for M103 & 105)
subjects = excludeAltSessions(subjects);
...
if plots.group_performance
    ...
    params = struct('nRule1',5,'nRule2','min','colors', colors, 'lineWidth', 1.5);
    vars = {"pCorrect",["pCorrect_congruent", "pCorrect_conflict"]};
%         ["maxCorrectMoving_congruent", "maxCorrectMoving_conflict"]}; %troubleshoot mean>max_moving...
    for i = 1:numel(vars)
        figs(i) = fig_periswitch_group(subjects,vars{i},params);
    end
    save_multiplePlots(figs, saveDir);
    clearvars figs;
end
