...
%Plot Individual Longitudinal Performance
if plots.longitudinal_licking && numel(subjects.sessions)>1
    %Full performance data for each subject
    saveDir = fullfile(dirs.results,'Longitudinal Licking');
    ...
    vars = {...
        ...
        ["normLicksCSPlus", "normLicksCSMinus"]... %for presentation
       };

    params = struct('colors', colors, 'lineWidth', 1.5,...
        'markerSize', 6,'omitShaping',true,...
        'figName',{"normLicksbyCueType","meanLicksbyCueType","normLicksbyCS"},...
        'displayNames',{...
    ...
    for i = 1:numel(vars)
        figs(i) = fig_longitudinal_licking(subjects, vars{i}, params(i));
    end
    save_multiplePlots(figs,saveDir);
    clearvars figs;
end
    ...
    for i = 1:numel(subjects)
        ...
        %Pavlovian Track
            ...
            if exist("figs") && ~isempty(figs)
                saveDir = fullfile(dirs.results,'session-summary','Pavlovian Track');
            save_multiplePlots(figs, saveDir);
            end
        % end
