...
function figs = plot_eventKernel_byBehVar( cellStruct, sessions, varName, sortBy )
...
for i = 1:numel(cellStruct) %Loop through cells
    ...
    %Generate figure 
    figs(i) = plot_trialAvgTimeseries(panel, {panel.title}, xLabel, yLabel, panel(1).tickLabelFormat, legend_loc);

    %Abbreviate variables for filename
    idx = regexp(sortBy,'(_)');
    psyName = ['psy', upper(sortBy{1}(idx(1)+1)), sortBy{1}(idx(1)+2:idx(end)-1)]; %truncate prefix 'psyTrack' and lose suffix 'meanCoef'
    
    figs(i).Name = join([varName, '-', psyName, '-cell', cellStruct(i).cellID],''); %Figure name
